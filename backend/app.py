...
def get_library_info():
    ...
    """
    info = {"version": controller.get_library_version()}
    return jsonify(info)
...
def create_backup_route():
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "Backup created successfully", "filename": backup_filename}
            ),
            201,
        )
...
def delete_backup_route(filename):
    ...
    if controller.delete_backup_file(filename):
        ...
        return (
            jsonify({"message": "Backup file not found or could not be deleted"}),
            404,
        )
...
def restore_from_backup_route():
    ...
    if controller.restore_from_file(file):
        ...
        return (
            jsonify({"message": "Restore failed. Invalid file or server error."}),
            500,
        )
...
def restore_from_server_backup_route(filename):
    ...
    if controller.restore_from_backup(filename):
        ...
        return (
            jsonify(
                {"message": "Restore failed. Backup file not found or server error."}
            ),
            500,
        )
