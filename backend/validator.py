    ...
    def _validate_duplicate_ids(self):
        ...
        for std in self.library.standards:
            if std.id in seen_ids:
                self.errors.append(
                    ValidationError("error", std.id, "Duplicate standard ID")
                )
            seen_ids.add(std.id)
        ...
        for cluster in self.library.clusters:
            if cluster.id in seen_cluster_ids:
                self.errors.append(
                    ValidationError(
                        "error", f"CLUSTER:{cluster.id}", "Duplicate cluster ID"
                    )
                )
    ...
    def _check_missing_rationales(self):
        ...
        for std in self.library.standards:
            ...
            if std.mac_vector.family > 0.1 and not rat.family_rationale.strip():
                self.errors.append(
                    ValidationError(
                        "warning", std.id, "Missing family_rationale (MAC value > 0.1)"
                    )
                )
            if std.mac_vector.group > 0.1 and not rat.group_rationale.strip():
                self.errors.append(
                    ValidationError(
                        "warning", std.id, "Missing group_rationale (MAC value > 0.1)"
                    )
                )
            if (
                std.mac_vector.reciprocity > 0.1
                and not rat.reciprocity_rationale.strip()
            ):
                self.errors.append(
                    ValidationError(
                        "warning",
            ...
            if std.mac_vector.heroism > 0.1 and not rat.heroism_rationale.strip():
                self.errors.append(
                    ValidationError(
                        "warning", std.id, "Missing heroism_rationale (MAC value > 0.1)"
                    )
                )
